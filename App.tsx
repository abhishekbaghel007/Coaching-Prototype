...
export default function App() {
  ...
  useEffect(() => {
    ...
    const savedAppearance = localStorage.getItem(STORAGE.appearance);
    setAppearance(savedAppearance === 'light' || savedAppearance === 'system' ? savedAppearance : 'dark');
    ...
    const hydrate = async (nextUser: User | null) => {
      ...
      try {
        ...
        if (pendingMigration) {
          ...
          await upsertProfile(nextUser, { target: localTarget, theme, appearance: appearance === 'system' ? resolvedAppearance : appearance });
  ...
  useEffect(() => {
    if (!user || !cloudReady) return;
    const timer = window.setTimeout(() => {
      upsertProfile(user, { target, theme, appearance: appearance === 'system' ? resolvedAppearance : appearance }).catch(error => { console.error(error); setCloudError('Could not sync profile settings.'); });
    }, 500);
    return () => window.clearTimeout(timer);
  ...
  const mobilePolishCss = `
    ...
    *{box-sizing:border-box}
    ...
    /* Final shared visual layer: semantic surfaces keep every existing screen coherent. */
    .app{--app-bg:var(--np-bg);--app-panel:var(--np-surface);--app-panel-2:var(--np-surface-2);--app-line:var(--np-border);--app-muted:var(--np-muted);--control:var(--np-surface-2);--control-hover:var(--np-surface-3);color:var(--np-text);background:radial-gradient(900px 480px at 84% -120px,color-mix(in srgb,var(--accent) 10%,transparent),transparent 68%),var(--app-bg)}
    .app[data-appearance='light']{--app-bg:#f6f7f8;--app-panel:rgba(255,255,255,.88);--app-panel-2:#f0f2f4;--app-line:rgba(22,32,38,.10);--app-muted:#68747b;--control:#fff;--control-hover:#f1f3f5;background:radial-gradient(900px 460px at 84% -130px,color-mix(in srgb,var(--accent) 13%,transparent),transparent 67%),#f6f7f8}
    .app :is(button,input,select,textarea){-webkit-tap-highlight-color:transparent}
    .app button{transition:transform .18s cubic-bezier(.2,.8,.2,1),background .18s ease,border-color .18s ease,box-shadow .18s ease,color .18s ease}
    .app button:not(:disabled):active{transform:scale(.975)}
    .app button:focus-visible,.app input:focus-visible,.app select:focus-visible,.app textarea:focus-visible{outline:3px solid color-mix(in srgb,var(--accent) 30%,transparent);outline-offset:2px}
    .top-nav{background:color-mix(in srgb,var(--app-panel) 88%,transparent);border-bottom-color:var(--app-line);box-shadow:0 1px 0 color-mix(in srgb,var(--np-text) 2%,transparent)}
    .top-nav nav button.active,.top-nav nav button:hover,.nav-theme:hover,.nav-search:hover{background:color-mix(in srgb,var(--accent) 10%,var(--control));color:var(--np-text)}
    .nav-theme,.nav-search,.nav-bell,.nav-login,.nav-avatar{background:color-mix(in srgb,var(--control) 88%,transparent);border-color:var(--app-line);color:var(--np-text)}
    .primary-button{box-shadow:0 10px 26px color-mix(in srgb,var(--accent) 20%,transparent)}
    .primary-button:hover{transform:translateY(-1px);box-shadow:0 14px 30px color-mix(in srgb,var(--accent) 28%,transparent)}
    .secondary-button{background:var(--control);border-color:var(--app-line);color:var(--np-text)}
    .secondary-button:hover{background:var(--control-hover)}
    .question-row,.history-row,.revision-card,.subject-card,.metric-card,.performance-panel,.builder-card,.mock-rules-card,.empty-state,.filter-panel{background:var(--app-panel);border-color:var(--app-line);box-shadow:0 10px 30px color-mix(in srgb,var(--np-text) 3%,transparent),inset 0 1px color-mix(in srgb,#fff 30%,transparent)}
    .question-row:hover,.history-row:hover,.revision-card:hover,.subject-card:hover{background:var(--app-panel-2);border-color:color-mix(in srgb,var(--accent) 26%,var(--app-line));transform:translateY(-2px)}
    .search-field,.filter-row select{background:var(--control);border-color:var(--app-line);color:var(--np-text)}
    .q-number,.mini-symbol{background:color-mix(in srgb,var(--accent) 9%,var(--control))}
    .progress-track,.m2-bar{background:color-mix(in srgb,var(--np-text) 8%,transparent)}
    .modal-backdrop,.result-overlay{background:color-mix(in srgb,#071013 67%,transparent);backdrop-filter:blur(22px) saturate(125%)}
    .modal,.result-sheet{background:var(--app-panel);border-color:var(--app-line);box-shadow:0 32px 100px rgba(0,0,0,.34),inset 0 1px color-mix(in srgb,#fff 26%,transparent)}
    .app[data-appearance='light'] .modal,.app[data-appearance='light'] .result-sheet{box-shadow:0 28px 80px rgba(20,30,36,.18),inset 0 1px rgba(255,255,255,.9)}
    .close-button,.mode-toggle button,.theme-swatches button,.profile-actions button,.report-options button{background:var(--control);border-color:var(--app-line);color:var(--np-text)}
    .mode-toggle{grid-template-columns:repeat(3,1fr)}
    .mode-toggle button.selected{background:var(--accent);color:#071013;border-color:transparent;box-shadow:0 8px 20px color-mix(in srgb,var(--accent) 22%,transparent)}
    .toast{background:color-mix(in srgb,var(--app-panel) 92%,transparent);border-color:var(--app-line);color:var(--np-text);box-shadow:0 18px 45px rgba(0,0,0,.22)}
    .auth-modal input,.note-modal textarea{background:var(--control);border-color:var(--app-line);color:var(--np-text)}
    .practice-question-card,.practice-option,.cbt-option{background:var(--app-panel);border-color:var(--app-line);color:var(--np-text)}
    .practice-option-number,.cbt-option .option-radio{background:var(--control);color:var(--app-muted)}
    .practice-option:hover,.cbt-option:hover{background:var(--app-panel-2)}
    .app[data-appearance='light'] .practice-filterbar,.app[data-appearance='light'] .practice-topbar{background:rgba(255,255,255,.94);border-color:var(--app-line)}
    .app[data-appearance='light'] .practice-option-copy,.app[data-appearance='light'] .cbt-option-label,.app[data-appearance='light'] .practice-options-heading strong,.app[data-appearance='light'] .cbt-options-heading strong{color:var(--np-text)}
    @media(max-width:760px){.mobile-bottom{background:color-mix(in srgb,var(--app-panel) 90%,transparent);border-color:var(--app-line);box-shadow:0 18px 50px color-mix(in srgb,var(--np-text) 20%,transparent),inset 0 1px color-mix(in srgb,#fff 24%,transparent)}.mobile-bottom button{color:var(--app-muted)}.mobile-bottom button.active{background:color-mix(in srgb,var(--accent) 10%,transparent);color:var(--accent)}.modal-backdrop{align-items:end;padding:10px}.modal,.result-sheet{width:100%!important;max-height:min(82dvh,720px);overflow:auto;border-radius:24px 24px 18px 18px}.toast{bottom:calc(92px + env(safe-area-inset-bottom));max-width:calc(100% - 32px);text-align:center}.page-title p{color:var(--app-muted)}}
    @media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;transition:none!important;animation:none!important}}
  `;
  return <div className="app" data-theme={theme} data-appearance={resolvedAppearance} style={{ '--accent': currentTheme.color } as CSSProperties}>
