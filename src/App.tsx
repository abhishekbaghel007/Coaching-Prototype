...
function ExamSetupView({ config, subjects, onToggleSubject, onChangeConfig, onBack, onStart }: {
  config: ExamConfig;
  subjects: string[];
  ...
  return (
    <section className="exam-setup">
      ...
      <div className="exam-hero">
        <div>
          <div className="section-label">KAZAN MBBS / EXAM MODE</div>
          <h2>Build your own mock exam.</h2>
          <p>Choose exactly what you want to test. Answers stay hidden until submission, just like a proper exam. Humanity briefly remembers what discipline is.</p>
        </div>
        <div className="exam-preview-card">
          <span>YOUR MOCK</span>
          <b>{config.questionCount}</b>
          <small>questions</small>
          <div>{config.timeMinutes} min · {config.subjects.length} subject{config.subjects.length === 1 ? '' : 's'}</div>
        </div>
      </div>
      ...
      <div className="exam-builder">
        ...
        <aside className="exam-rules">
          ...
          <button className="next-btn exam-start-btn" disabled={!config.subjects.length || !poolSize} onClick={onStart}>START EXAM <span>→</span></button>
          <p className="exam-note">You can leave before starting without losing anything.</p>
        </aside>
      </div>
    </section>
  );
