      ...
      <div className="practice-shell">
        ...
        <main className="practice-content">
          <div className="practice-question-card">
            ...
            <div className="practice-question-text">{current.question.replace(/\s+/g, ' ').trim()}</div>
          ...
          <div className="practice-options">
            {OPTION_LABELS.map((label, i) => {
              ...
              return <button key={label} className={`practice-option ${isSelected ? 'selected' : ''} ${showResult && isCorrect ? 'correct' : ''} ${showResult && isSelected && !isCorrect ? 'wrong' : ''}`} onClick={() => chooseAnswer(i)} disabled={current.status === 'dropped'}>
                <span className="practice-option-number">{i + 1}</span><span className="practice-option-copy"><b>{label}</b>{current.options[i] && current.options[i] !== label ? current.options[i] : `Choice ${label}`}</span><span className="practice-option-state">{showResult && isCorrect ? '✓' : isSelected ? '●' : ''}</span>
              </button>;
            })}
          </div>
