    ...
    const hydrate = async (nextUser: User | null) => {
      ...
      try {
        ...
        if (pendingMigration) {
          ...
          const mergedAnswers = [...snapshot.answers, ...allLocalAnswers].filter((item, index, arr) => arr.findIndex(x => x.id === item.id) === index);
          ...
          const localAnswers = read<AnswerRecord[]>(STORAGE.answers, []);
          const mergedAnswers = [...snapshot.answers, ...localAnswers].filter((item, index, arr) => arr.findIndex(x => x.id === item.id) === index);
