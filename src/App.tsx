...
export default function App() {
  ...
  useEffect(() => {
    // Keep the web app inside its own navigation history. This is intentionally
    // independent of React's tab state so Android/browser back cannot close the app.
    const marker = '__neetprep_back_guard__';
    let handling = false;
    const arm = () => {
      const state = window.history.state;
      if (!state || state.__neetprepBackGuard !== marker) {
        window.history.replaceState({ ...(state || {}), __neetprepBackGuard: marker, __neetprepGuardLayer: 1 }, '', window.location.href);
      }
      window.history.pushState({ __neetprepBackGuard: marker, __neetprepGuardLayer: 2 }, '', window.location.href);
    };
    const openMainMenu = () => {
      if (handling) return;
      handling = true;
      ...
      window.setTimeout(() => {
        try { window.history.pushState({ __neetprepBackGuard: marker, __neetprepGuardLayer: 2 }, '', window.location.href); } finally { handling = false; }
      }, 0);
    };
    const onPopState = () => openMainMenu();
    const onHashChange = () => openMainMenu();
    ...
    document.addEventListener('backbutton', onNativeBack as EventListener, false);
    return () => {
      window.removeEventListener('popstate', onPopState);
      window.removeEventListener('hashchange', onHashChange);
      document.removeEventListener('backbutton', onNativeBack as EventListener, false);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    meta.content = appearance === 'light' ? '#f5f5f7' : '#080b0e';
  ...
  const mobilePolishCss = `
    ...
    *{box-sizing:border-box}
    ...
    .page-title h1{font-size:56px;letter-spacing:-2.8px}.page-title p{font-size:14px}.filter-panel{border-radius:20px;padding:10px;background:color-mix(in srgb,var(--text) 3%,transparent)}.question-row,.history-row,.revision-card,.subject-card,.metric-card,.performance-panel,.builder-card,.mock-rules-card,.empty-state{border-radius:20px;background:color-mix(in srgb,var(--text) 3.5%,transparent);border-color:color-mix(in srgb,var(--text) 8%,transparent);box-shadow:none}.question-row:hover,.history-row:hover,.revision-card:hover,.subject-card:hover{transform:none;background:color-mix(in srgb,var(--text) 6%,transparent)}
    .primary-button{background:var(--apple-blue);color:#fff;border-radius:14px;box-shadow:none}.secondary-button{border-radius:14px;background:color-mix(in srgb,var(--text) 5%,transparent)}
    .modal-backdrop{background:rgba(0,0,0,.5);backdrop-filter:blur(26px);-webkit-backdrop-filter:blur(26px)}.modal{border-radius:28px;background:color-mix(in srgb,var(--app-panel) 96%,transparent);border-color:color-mix(in srgb,var(--text) 9%,transparent);box-shadow:0 30px 100px rgba(0,0,0,.35);backdrop-filter:blur(28px)}
    .apple-appearance-backdrop{place-items:end center;padding:0}.apple-appearance{width:min(520px,100%);padding:22px 24px 24px;border-radius:30px 30px 0 0}.apple-sheet-handle{width:36px;height:4px;border-radius:99px;background:color-mix(in srgb,var(--text) 20%,transparent);margin:0 auto 21px}.apple-close{position:absolute;right:18px;top:18px;width:34px;height:34px;border:0;border-radius:50%;background:color-mix(in srgb,var(--text) 7%,transparent);color:var(--text);font-size:20px}.apple-appearance h2{font-size:30px;letter-spacing:-1.5px;margin:7px 0 20px}.apple-appearance-section{margin-top:19px}.apple-appearance-section>small{display:block;font-size:10px;letter-spacing:.13em;color:var(--app-muted);font-weight:900;margin:0 0 9px}.apple-mode-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.apple-mode-grid button,.apple-colour-grid button{min-height:62px;display:flex;align-items:center;gap:10px;border:1px solid color-mix(in srgb,var(--text) 9%,transparent);border-radius:16px;background:color-mix(in srgb,var(--text) 4%,transparent);color:var(--text);padding:0 13px;text-align:left}.apple-mode-grid button.selected,.apple-colour-grid button.selected{border-color:color-mix(in srgb,var(--accent) 65%,transparent);background:color-mix(in srgb,var(--accent) 11%,transparent)}.apple-mode-grid button>span{width:32px;height:32px;border-radius:11px;display:grid;place-items:center;background:color-mix(in srgb,var(--text) 7%,transparent);color:var(--accent)}.apple-mode-grid b,.apple-colour-grid b{font-size:12px;flex:1}.apple-mode-grid i,.apple-colour-grid i{font-style:normal;color:var(--accent);font-size:19px}.apple-colour-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.apple-colour-grid button{min-height:50px}.apple-colour-grid button>span{width:17px;height:17px;border-radius:50%;box-shadow:0 0 18px color-mix(in srgb,currentColor 25%,transparent)}.apple-preference-note{margin-top:16px;padding-top:14px;border-top:1px solid color-mix(in srgb,var(--text) 8%,transparent);color:var(--app-muted);font-size:10px}
    ...
    /* APPLE UI — system back navigation + account access */
    .back-menu-backdrop{position:fixed;inset:0;z-index:500;display:grid;place-items:end center;padding:18px;background:rgba(0,0,0,.46);backdrop-filter:blur(24px) saturate(140%);-webkit-backdrop-filter:blur(24px) saturate(140%)}
    .back-menu{width:min(560px,100%);border:1px solid color-mix(in srgb,var(--text) 10%,transparent);border-radius:30px;background:color-mix(in srgb,var(--app-panel) 96%,transparent);box-shadow:0 28px 100px rgba(0,0,0,.42),inset 0 1px rgba(255,255,255,.04);padding:10px 18px max(18px,env(safe-area-inset-bottom));backdrop-filter:blur(32px) saturate(150%);-webkit-backdrop-filter:blur(32px) saturate(150%)}
    .back-menu-handle{width:38px;height:4px;border-radius:99px;background:color-mix(in srgb,var(--text) 20%,transparent);margin:4px auto 18px}.back-menu-head{display:flex;align-items:flex-start;justify-content:space-between;padding:0 2px 16px;border-bottom:1px solid color-mix(in srgb,var(--text) 8%,transparent)}.back-menu-head h2{margin:5px 0 0;font-size:26px;letter-spacing:-1.2px}.back-menu-close{width:34px;height:34px;border:0;border-radius:50%;background:color-mix(in srgb,var(--text) 7%,transparent);color:var(--text);font-size:20px;cursor:pointer}.back-menu-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;padding:16px 0}.back-menu-grid button{min-height:74px;border:1px solid color-mix(in srgb,var(--text) 8%,transparent);border-radius:18px;background:color-mix(in srgb,var(--text) 4%,transparent);color:var(--text);display:flex;flex-direction:column;align-items:flex-start;justify-content:space-between;padding:12px;cursor:pointer;text-align:left}.back-menu-grid button span{width:31px;height:31px;border-radius:11px;display:grid;place-items:center;background:color-mix(in srgb,var(--apple-blue) 12%,transparent);color:var(--apple-blue);font-size:17px}.back-menu-grid button b{font-size:11px}.back-menu-grid button.active{border-color:color-mix(in srgb,var(--apple-blue) 55%,transparent);background:color-mix(in srgb,var(--apple-blue) 8%,transparent)}.back-menu-account{padding-top:2px}.back-menu-account button{width:100%;display:grid;grid-template-columns:42px 1fr 20px;align-items:center;gap:11px;border:0;border-top:1px solid color-mix(in srgb,var(--text) 8%,transparent);padding:15px 2px 4px;background:transparent;color:var(--text);text-align:left;cursor:pointer}.back-menu-account button>span{width:42px;height:42px;border-radius:50%;display:grid;place-items:center;background:var(--apple-blue);color:#fff;font-weight:900}.back-menu-account b{display:block;font-size:12px}.back-menu-account small{display:block;color:var(--app-muted);font-size:9px;margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.back-menu-account strong{font-size:22px;font-weight:300;color:var(--app-muted)}
    @media(min-width:761px){.back-menu-backdrop{place-items:center}.back-menu{padding-bottom:20px}}

    /* APPLE UI 2.1 — targeted screenshot fixes
       Visual-only overrides. Existing CBT/practice behavior is untouched. */
    .app:has(.cbt-shell) .mobile-bottom,
    ...
    .cbt-topbar{
      ...
      background:color-mix(in srgb,var(--app-bg) 82%,transparent)!important;
    ...
    .cbt-option{
      ...
      background:linear-gradient(180deg,
        ...
        color-mix(in srgb,var(--text) 2.5%,transparent))!important;
      border:1px solid color-mix(in srgb,var(--text) 9%,transparent)!important;
    ...
    .cbt-option::after{
      ...
      inset:0;
      pointer-events:none;
      background:linear-gradient(110deg,rgba(255,255,255,.055),transparent 38%);
      opacity:.55;
    }
    .cbt-option:hover{transform:translateY(-1px)!important}
    .cbt-option.selected{
      background:linear-gradient(180deg,
        color-mix(in srgb,var(--apple-blue) 13%,transparent),
        color-mix(in srgb,var(--apple-blue) 7%,transparent))!important;
      border-color:color-mix(in srgb,var(--apple-blue) 58%,transparent)!important;
      box-shadow:inset 0 1px rgba(255,255,255,.08),
                 0 8px 22px color-mix(in srgb,var(--apple-blue) 10%,transparent)!important;
    }

    /* Mobile CBT action dock: one balanced control, not five unrelated buttons */
    @media(max-width:760px){
      .cbt-shell{min-height:100dvh!important;padding-bottom:0!important}
      .cbt-topbar{position:sticky!important}
      .cbt-layout{padding:16px 14px 118px!important}
      .cbt-question-area{padding-bottom:0!important}
      .cbt-question-card{margin-bottom:0!important}
      .cbt-actions-mobile{
        ...
        grid-template-columns:42px 42px 42px minmax(0,1fr) 42px!important;
        gap:5px!important;
        align-items:center!important;
        padding:6px!important;
        border-radius:21px!important;
        background:color-mix(in srgb,var(--app-panel) 90%,transparent)!important;
        border:1px solid color-mix(in srgb,var(--text) 11%,transparent)!important;
      ...
      .cbt-mobile-tool:active,
      .cbt-mobile-end:active{transform:scale(.94)!important}
      .cbt-mobile-tool.active{
        background:color-mix(in srgb,var(--apple-blue) 12%,transparent)!important;
      ...
      .cbt-mobile-primary{
        ...
        color:#fff!important;
        box-shadow:0 6px 18px color-mix(in srgb,var(--apple-blue) 22%,transparent)!important;
        font-size:13px!important;
        letter-spacing:-.1px!important;
      }
      .cbt-mobile-end{
        background:color-mix(in srgb,var(--text) 6%,transparent)!important;
        border:1px solid color-mix(in srgb,var(--text) 7%,transparent)!important;
    ...
    /* Practice solver bottom controls */
    .practice-actionbar{
      border:1px solid color-mix(in srgb,var(--text) 10%,transparent)!important;
      background:color-mix(in srgb,var(--app-panel) 91%,transparent)!important;
      box-shadow:0 18px 55px rgba(0,0,0,.25),inset 0 1px rgba(255,255,255,.05)!important;
    ...
    .solver-action-icon span{font-size:17px!important}
    .solver-action-icon b{font-size:9px!important;font-weight:700!important}
    .solver-action-icon:hover,
    .solver-action-icon.active{
    ...
    .practice-next{
      ...
      background:var(--apple-blue)!important;
      color:#fff!important;
      box-shadow:0 7px 20px color-mix(in srgb,var(--apple-blue) 20%,transparent)!important;
      font-size:12px!important;
      font-weight:850!important;
      padding:0 18px!important;
    }
    .practice-next b{font-size:17px!important;font-weight:450!important}
    @media(max-width:760px){
      .practice-shell{min-height:100dvh!important}
      .practice-content{padding-bottom:112px!important}
      .practice-actionbar{
        position:fixed!important;
        left:12px!important;
        right:12px!important;
      ...
      .practice-action-group{display:flex!important;gap:2px!important}
      .solver-action-icon{
      ...
      .solver-action-icon span{font-size:16px!important}
      .solver-action-icon b{font-size:8px!important}
      .solver-more{display:grid!important}
      .practice-next{
        width:100%!important;
        min-width:0!important;
        height:46px!important;
      ...
      html,body,#root{background:var(--app-bg,#080b0e)!important;color:var(--text,#f5f5f7)!important;overscroll-behavior-y:none!important}
      .app{background:var(--app-bg,#080b0e)!important;min-height:100dvh!important}
      @media(max-width:700px){
        html,body,#root{background:var(--app-bg,#080b0e)!important}
        body{margin:0!important;padding:0!important}
        .v7-top{padding-top:max(10px,env(safe-area-inset-top))!important;min-height:58px!important;box-sizing:content-box!important;background:color-mix(in srgb,var(--app-bg) 94%,transparent)!important}
      }
      /* FINAL APPLE PASS 4: status bar + system back + native surfaces */
    :root{background:var(--app-bg)!important}
    html,body,#root{background:var(--app-bg)!important;overscroll-behavior-y:none!important}
    body{min-height:100dvh!important;margin:0!important;padding:0!important}
    .app{min-height:100dvh!important;background:radial-gradient(900px 500px at 50% -140px,color-mix(in srgb,var(--accent) 7%,transparent),transparent 68%),var(--app-bg)!important}
    .mobile-bottom{background:color-mix(in srgb,var(--app-panel) 92%,transparent)!important;border:1px solid color-mix(in srgb,var(--text) 10%,transparent)!important;box-shadow:0 18px 55px rgba(0,0,0,.34),inset 0 1px rgba(255,255,255,.055)!important}
    .mobile-bottom .mobile-plus{width:48px!important;height:48px!important;margin-top:-7px!important;border-radius:50%!important;background:var(--apple-blue)!important;color:#fff!important;box-shadow:0 8px 24px color-mix(in srgb,var(--apple-blue) 30%,transparent),0 0 0 5px var(--app-bg)!important;font-size:0!important}
    .mobile-bottom .mobile-plus span{width:48px!important;height:48px!important;display:grid!important;place-items:center!important;font-size:20px!important;font-weight:500!important}
    .cbt-shell{background:var(--app-bg)!important;color:var(--text)!important;min-height:100dvh!important}
    .cbt-actions-mobile{padding:6px!important;border-radius:22px!important;background:color-mix(in srgb,var(--app-panel) 94%,transparent)!important;border-color:color-mix(in srgb,var(--text) 10%,transparent)!important;box-shadow:0 16px 50px rgba(0,0,0,.34),inset 0 1px rgba(255,255,255,.05)!important}
    .cbt-mobile-tool,.cbt-mobile-end{border-radius:15px!important}
    .cbt-mobile-primary{border-radius:15px!important;box-shadow:0 7px 18px color-mix(in srgb,var(--apple-blue) 22%,transparent)!important}
    .cbt-option{transition:transform .16s ease,border-color .16s ease,background .16s ease!important}
    .cbt-option:active{transform:scale(.992)!important}
    .cbt-palette{background:color-mix(in srgb,var(--app-panel) 96%,transparent)!important}
    .back-menu-backdrop{background:color-mix(in srgb,var(--app-bg) 82%,#000 18%)!important}
    .back-menu{background:color-mix(in srgb,var(--app-panel) 98%,transparent)!important}
    @media(max-width:760px){
      .top-nav{display:none!important}
      .app{padding-top:env(safe-area-inset-top)!important}
      .student-home-stage{padding-left:16px!important;padding-right:16px!important}
      .mobile-bottom{bottom:max(10px,env(safe-area-inset-bottom))!important}
    }

    /* FINAL BACK/STATUS/OVERLAY POLISH */
      .back-menu-backdrop{background:color-mix(in srgb,var(--app-bg) 72%,#000 28%)!important;backdrop-filter:blur(34px) saturate(135%)!important;-webkit-backdrop-filter:blur(34px) saturate(135%)!important}
      .back-menu{background:color-mix(in srgb,var(--app-panel) 96%,transparent)!important;border:1px solid color-mix(in srgb,var(--text) 10%,transparent)!important;box-shadow:0 30px 100px rgba(0,0,0,.48)!important}
  ...
  return <div className="app" data-theme={theme} data-appearance={appearance} style={{ '--accent': currentTheme.color } as CSSProperties}>
    ...
    <nav className="mobile-bottom"><button className={tab === 'home' ? 'active' : ''} onClick={() => go('home')}><span><MobileIcon name="home" /></span><b>Home</b></button><button className={tab === 'practice' ? 'active' : ''} onClick={() => go('practice')}><span><MobileIcon name="practice" /></span><b>Practice</b></button><button className="mobile-plus" aria-label="Quick practice" onClick={() => startAttempt(questions.filter(q => q.status !== 'dropped').slice(0, 10), 'practice', 'Quick practice')}><span aria-hidden="true">✦</span></button><button className={tab === 'mocks' ? 'active' : ''} onClick={() => go('mocks')}><span><MobileIcon name="test" /></span><b>Tests</b></button><button className={tab === 'progress' ? 'active' : ''} onClick={() => go('progress')}><span><MobileIcon name="progress" /></span><b>Progress</b></button></nav>
    {solver && renderSolver()}
    {result && renderResult()}
    {backMenuOpen && <div className="back-menu-backdrop" onMouseDown={() => setBackMenuOpen(false)}>
      <div className="back-menu" onMouseDown={e => e.stopPropagation()}>
        <div className="back-menu-handle" />
        <div className="back-menu-head"><div><span className="section-kicker">NEETPREP</span><h2>Where next?</h2></div><button className="back-menu-close" onClick={() => setBackMenuOpen(false)} aria-label="Close navigation">×</button></div>
        <div className="back-menu-grid">
          {([['home','Home'],['practice','Practice'],['mocks','Tests'],['saved','Saved'],['mistakes','Mistakes'],['progress','Progress']] as [Tab,string][]).map(([id,label]) => <button key={id} className={tab === id ? 'active' : ''} onClick={() => { setBackMenuOpen(false); go(id); }}><span>{label === 'Home' ? '⌂' : label === 'Practice' ? '◉' : label === 'Tests' ? '▤' : label === 'Saved' ? '▢' : label === 'Mistakes' ? '◌' : '↗'}</span><b>{label}</b></button>)}
        </div>
        <div className="back-menu-account"><button onClick={() => { setBackMenuOpen(false); openAccount(); }}><span>{user ? (user.email?.[0] ?? 'A').toUpperCase() : 'A'}</span><div><b>{user ? 'Account' : 'Sign in'}</b><small>{user ? user.email : 'Sync your progress across devices'}</small></div><strong>›</strong></button></div>
      </div>
    </div>}
