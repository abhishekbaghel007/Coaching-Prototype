...
export default function App() {
  ...
  const [adminChecking, setAdminChecking] = useState(isAdminPath);
  ...
  useEffect(() => {
    ...
    async function checkTeacherSession() {
      ...
      try {
        ...
        if (cancelled) return;
        if (role === 'teacher' || role === 'admin') setAdminUser(data.user);
        else {
          await supabase.auth.signOut();
          setAdminUser(null);
          setAdminAuthError('This account does not have teacher access.');
        }
      } catch (error) {
        if (!cancelled) setAdminAuthError(error instanceof Error ? error.message : 'Could not verify teacher access.');
      } finally {
        if (!cancelled) setAdminChecking(false);
      }
    }
    void checkTeacherSession();
  ...
  async function teacherSignIn() {
    ...
    if (!cleanEmail || !adminPassword) {
      setAdminAuthError('Enter the teacher email and password.');
      return;
    }
    setAdminAuthBusy(true);
    setAdminAuthError('');
    try {
      const { data, error } = await supabase.auth.signInWithPassword({ email: cleanEmail, password: adminPassword });
      if (error || !data.user) throw new Error(error?.message || 'Could not sign in.');
      const role = await getStaffRole(data.user.id);
      if (role !== 'teacher' && role !== 'admin') {
        await supabase.auth.signOut();
        throw new Error('This account is not assigned teacher access.');
      }
      setAdminUser(data.user);
      setAdminPassword('');
    } catch (error) {
      setAdminAuthError(error instanceof Error ? error.message : 'Could not sign in.');
    } finally {
  ...
  const teacherGateCss = `
    .teacher-gate{min-height:100vh;display:grid;place-items:center;padding:28px;background:#070a0d;color:#eef6f5;font-family:Manrope,system-ui,sans-serif;position:relative;overflow:hidden}
    .teacher-gate:before{content:'';position:absolute;width:520px;height:520px;left:-240px;top:-220px;border-radius:50%;background:radial-gradient(circle,rgba(88,208,192,.13),transparent 68%);pointer-events:none}
    .teacher-gate:after{content:'';position:absolute;width:420px;height:420px;right:-190px;bottom:-180px;border-radius:50%;background:radial-gradient(circle,rgba(109,168,255,.09),transparent 70%);pointer-events:none}
    .teacher-gate-card{width:min(100%,470px);position:relative;z-index:1;padding:38px;border:1px solid rgba(255,255,255,.09);border-radius:34px;background:linear-gradient(145deg,rgba(22,28,31,.92),rgba(10,14,17,.94));box-shadow:0 35px 100px rgba(0,0,0,.42),inset 0 1px rgba(255,255,255,.06);backdrop-filter:blur(30px);-webkit-backdrop-filter:blur(30px)}
    .teacher-login-card{padding:34px 34px 25px}.teacher-gate-top{display:flex;align-items:center;justify-content:space-between;margin-bottom:38px}.teacher-brand-mark{width:46px;height:46px;border-radius:15px;display:grid;place-items:center;background:linear-gradient(145deg,#79e0d3,#43bfb1);color:#06100f;font:900 17px Manrope;box-shadow:0 12px 30px rgba(88,208,192,.18),inset 0 1px rgba(255,255,255,.45)}.teacher-secure{font-size:9px;letter-spacing:.14em;font-weight:800;color:#6fcfc3}.teacher-gate-card .section-kicker{display:block;color:#67cfc3;font-size:9px;letter-spacing:.16em;font-weight:900}.teacher-gate-card h1{margin:12px 0 8px;font-size:42px;line-height:1;letter-spacing:-.06em}.teacher-gate-copy,.teacher-gate-card>p{color:#91a0a5;font-size:13px;line-height:1.55;margin:0}.teacher-login-form{display:grid;gap:17px;margin-top:28px}.teacher-login-form label{display:grid;gap:7px;color:#c7d0d2;font-size:11px;font-weight:800;letter-spacing:.03em}.teacher-login-form input{width:100%;height:53px;border:1px solid rgba(255,255,255,.09);border-radius:16px;background:rgba(255,255,255,.045);color:#eef6f5;padding:0 15px;outline:none;font:600 14px Manrope;transition:.2s}.teacher-login-form input:focus{border-color:rgba(88,208,192,.55);box-shadow:0 0 0 4px rgba(88,208,192,.08)}.teacher-password{position:relative}.teacher-password input{padding-right:70px}.teacher-password button{position:absolute;right:9px;top:9px;height:35px;padding:0 10px;border:0;border-radius:10px;background:rgba(255,255,255,.06);color:#9eadaf;font:800 10px Manrope;cursor:pointer}.teacher-auth-error{padding:11px 13px;border:1px solid rgba(255,112,112,.2);border-radius:13px;background:rgba(255,112,112,.07);color:#ffb4b4;font-size:11px;line-height:1.4}.teacher-login-button{height:56px;border:0;border-radius:17px;background:linear-gradient(135deg,#68d9cc,#43bfb1);color:#06100f;font:900 13px Manrope;letter-spacing:-.01em;cursor:pointer;box-shadow:0 15px 35px rgba(88,208,192,.18);transition:transform .2s,filter .2s}.teacher-login-button:hover{transform:translateY(-1px);filter:brightness(1.04)}.teacher-login-button:disabled{opacity:.65;cursor:wait}.teacher-login-button span{float:right;font-size:17px}.teacher-gate-footer{display:flex;align-items:center;justify-content:space-between;gap:15px;margin-top:22px;padding-top:19px;border-top:1px solid rgba(255,255,255,.07);color:#68767a;font-size:9px;line-height:1.4}.teacher-gate-footer button{border:0;background:none;color:#8fd9d1;font:800 10px Manrope;cursor:pointer;white-space:nowrap}.teacher-loader{display:flex;gap:6px;margin-top:22px}.teacher-loader i{width:6px;height:6px;border-radius:50%;background:#67d5c9;animation:teacherPulse 1.1s infinite ease-in-out}.teacher-loader i:nth-child(2){animation-delay:.15s}.teacher-loader i:nth-child(3){animation-delay:.3s}@keyframes teacherPulse{0%,80%,100%{opacity:.25;transform:scale(.7)}40%{opacity:1;transform:scale(1)}}
    @media(max-width:520px){.teacher-gate{padding:16px}.teacher-gate-card{padding:28px 22px 22px;border-radius:28px}.teacher-gate-card h1{font-size:36px}.teacher-gate-top{margin-bottom:30px}.teacher-gate-footer{align-items:flex-start;flex-direction:column}}
  `;

  if (isAdminPath) {
    if (adminChecking) {
      return <><style>{teacherGateCss}</style><div className="teacher-gate"><div className="teacher-gate-card"><div className="teacher-brand-mark">N</div><span className="section-kicker">NEETPREP • STAFF</span><h1>Securing your workspace.</h1><p>Checking your teacher credentials…</p><div className="teacher-loader"><i/><i/><i/></div></div></div></>;
    }
    if (adminUser) {
      return <AdminPanel user={adminUser} onExit={() => { window.location.href = '/'; }} />;
    }
    return <><style>{teacherGateCss}</style><div className="teacher-gate">
      <div className="teacher-gate-card teacher-login-card">
        <div className="teacher-gate-top"><div className="teacher-brand-mark">N</div><span className="teacher-secure">SECURE STAFF ACCESS</span></div>
        <span className="section-kicker">NEETPREP • TEACHER COMMAND</span>
        <h1>Welcome back.</h1>
        <p className="teacher-gate-copy">Sign in with the teacher account created by your administrator.</p>
        <form onSubmit={e => { e.preventDefault(); void teacherSignIn(); }} className="teacher-login-form">
          <label>Email<input type="email" value={adminEmail} onChange={e => setAdminEmail(e.target.value)} placeholder="teacher@yourcoaching.com" autoComplete="username" autoFocus /></label>
          <label>Password<div className="teacher-password"><input type={showAdminPassword ? 'text' : 'password'} value={adminPassword} onChange={e => setAdminPassword(e.target.value)} placeholder="Your password" autoComplete="current-password" /><button type="button" onClick={() => setShowAdminPassword(v => !v)}>{showAdminPassword ? 'Hide' : 'Show'}</button></div></label>
          {adminAuthError && <div className="teacher-auth-error">{adminAuthError}</div>}
          <button className="teacher-login-button" type="submit" disabled={adminAuthBusy}>{adminAuthBusy ? 'Signing in…' : 'Sign in to Teacher Console'} <span>↗</span></button>
        </form>
        <div className="teacher-gate-footer"><span>Teacher accounts are created privately.</span><button onClick={() => { window.location.href = '/'; }}>← Student app</button></div>
      </div>
    </div></>;
  }
