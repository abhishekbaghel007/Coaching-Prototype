...
export default function App() {
  ...
  useEffect(() => { if (!adminPath) return; supabase.auth.getUser().then(({ data }) => setAdminUser(data.user ?? null)); }, [adminPath]);
  if (adminPath) {
    if (!adminUser) return <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: '#090b0e', color: '#f2f5f7', fontFamily: 'Manrope, system-ui' }}><div style={{ textAlign: 'center' }}><div style={{ fontSize: 34, fontWeight: 900 }}>Teacher Console</div><p style={{ color: '#89939f' }}>Sign in with your teacher account to continue.</p><button style={{ border: 0, borderRadius: 11, padding: '11px 16px', fontWeight: 900, background: '#58d0c0' }} onClick={() => { window.location.href = '/'; }}>Open student app</button></div></div>;
