...
export type CloudSnapshot = {
  ...
  answers: Array<CloudAnswerRecord>;
  theme?: string | null;
...
export async function loadCloudSnapshot(user: User): Promise<CloudSnapshot | null> {
  ...
  const [profileRes, statesRes, attemptsRes, dailyRes, answersRes] = await Promise.all([
    ...
    supabase.from('question_attempts').select('id, question_id, selected_index, is_correct, subject, mode, answered_at').eq('user_id', user.id).order('answered_at', { ascending: true }).limit(10000),
  ]);

  const firstError = [profileRes.error, statesRes.error, attemptsRes.error, dailyRes.error, answersRes.error].find(Boolean);
  ...
  const answers = ((answersRes.data ?? []) as CloudAnswerRecord[]);
  ...
  return {
    ...
    notes,
    daily,
    target: profile?.target_score ?? 650,
    results,
    answers,
...
export async function syncAnswerRecords(user: User, records: Array<CloudAnswerRecord | { answeredAt: number; id: string; question_id: string; selected_index: number; is_correct: boolean; subject: 'Physics' | 'Chemistry' | 'Biology'; mode: 'practice' | 'mock' }>) {
  ...
  const rows = records.map(record => ({
    ...
    mode: record.mode,
    answered_at: 'answered_at' in record ? record.answered_at : new Date(record.answeredAt).toISOString(),
  }));
  const { error } = await supabase.from('question_attempts').upsert(rows, { onConflict: 'id' });
  if (error) throw error;
}
