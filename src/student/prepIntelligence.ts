...
type RankedSubject = { subject: Subject; attempted: number; correct: number; incorrect: number; accuracy: number };

function accuracyOf(stat: SubjectStats[Subject]) {
  return stat.attempted ? stat.correct / stat.attempted : 0;
}

function rankedSubjectStats(stats: SubjectStats): RankedSubject[] {
  return SUBJECT_ORDER
    .map(subject => ({ subject, ...stats[subject], accuracy: Math.round(accuracyOf(stats[subject]) * 100) }))
    .filter(item => item.attempted > 0)
    .sort((a, b) => a.accuracy - b.accuracy || b.attempted - a.attempted);
}

export function rankSubjects(stats: SubjectStats): Subject[] {
  return rankedSubjectStats(stats).map(item => item.subject);
}

function intelligenceResult(base: Omit<IntelligenceResult, 'title' | 'reason'>): IntelligenceResult {
  return { ...base, title: base.headline, reason: base.detail };
}

export function buildPrepIntelligence(stats: SubjectStats, mistakesCount: number, today: number, dailyGoal: number, target: number): IntelligenceResult {
  const ranked = rankedSubjectStats(stats);
  ...
  if (!weakest) {
    return intelligenceResult({
      subject: null, accuracy: 0, attempted: 0, recoveryMarks,
      ...
      action: 'practice', priority: 'medium',
    });
  }

  if (mistakesCount >= 3) {
    return intelligenceResult({
      subject: weakest.subject, accuracy: weakest.accuracy, attempted: weakest.attempted, recoveryMarks,
      ...
      action: 'repair', priority: weakest.accuracy < 60 ? 'high' : 'medium',
    });
  }
  ...
  return intelligenceResult({
    ...
    action: 'progress', priority: target >= 680 ? 'high' : 'low',
  });
}

export function buildScoreRecovery(stats: SubjectStats, mistakesCount: number): RecoveryResult {
  const weakest = rankedSubjectStats(stats)[0];
  const marks = Math.min(100, mistakesCount * 5);
  const estimatedQuestions = Math.min(20, mistakesCount);

  if (!mistakesCount) {
    return {
      marks: 0, recoverable: 0, mistakes: 0, estimatedQuestions: 0,
  ...
  return {
    marks, recoverable: marks, mistakes: mistakesCount, estimatedQuestions,
    subject: weakest?.subject ?? null,
